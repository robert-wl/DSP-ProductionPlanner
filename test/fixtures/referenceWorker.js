 ...
 *
 * Verbatim copy of src/Worker.js at commit 6afbc50, i.e. the last revision
 * before the calculation was optimized. test/differential.test.js runs this
 * side by side with the current src/Worker.js and requires both to produce
 * identical output, so the optimization stays behaviour preserving.
 *
 * If a change to src/Worker.js is *meant* to change the planner's output,
 * that is a deliberate decision: update this file and say so in the commit.
 */
